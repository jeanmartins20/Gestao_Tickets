...
class TicketApp:
   ...
   from matplotlib.backends.backend_tkagg import FigureCanvasTkAgg  # Adicione no início do arquivo se ainda não estiver
...
def _show_summary_and_chart(self):
    ...
    # Criação da nova janela fullscreen
    ...
    canvas.get_tk_widget().pack(fill='both', expand=True)
        ...
        resumo = tk.Toplevel(self.root)
        resumo.title("Resumo de Tickets")
        resumo.geometry("400x200")

        ttk.Label(resumo, text=f"Total de Tickets Hoje: {total_dia}", font=('Segoe UI', 12)).pack(pady=10)
        ttk.Label(resumo, text=f"Total na Semana: {total_semana}", font=('Segoe UI', 12)).pack(pady=10)
        ttk.Label(resumo, text=f"Total no Mês: {total_mes}", font=('Segoe UI', 12)).pack(pady=10)

        # Gráfico de desempenho acumulado
        plt.figure(figsize=(10, 5))
        df_sorted = df.sort_values(by='data')
        df_sorted['contagem'] = 1
        df_sorted['acumulado'] = df_sorted['contagem'].cumsum()
        plt.plot(df_sorted['data'], df_sorted['acumulado'], marker='o', linestyle='-', color='steelblue')
        plt.title("Desempenho Acumulado de Tickets")
        plt.xlabel("Data")
        plt.ylabel("Tickets Acumulados")
        plt.grid(True)
        plt.tight_layout()
